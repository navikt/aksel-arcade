...
The current SDK rebuild slice restores the Desktop MCP resource and tool surface: `resources/list`, `resources/templates/list`, and `resources/read` publish `arcade://desktop/*`, `arcade://aksel/*`, `arcade://project/*`, dynamic page source resources, dynamic page annotation resources at `arcade://project/pages/{pageId}/annotations`, and capture-produced `arcade://preview/captures/*` resources. `tools/list` and `tools/call` now expose `read_resource`, `read_source`, `list_annotations`, `watch_annotations`, `capture_preview_evidence`, `apply_changes`, `acknowledge_annotation`, `resolve_annotation`, `dismiss_annotation`, and `reply_to_annotation`. Arcade source is virtual `arcade://...` content, not repository-backed source content.
...
For the current smoke checklist, use an MCP client that exposes `resources/list`, `resources/templates/list`, `resources/read`, and `tools/call` so you can verify both the resource-driven guidance flow and the live mutation loop.
