...
Desktop MCP v1 publishes three tools — `read_resource`, `capture_preview_evidence`, and `apply_changes` — plus `arcade://desktop/*`, `arcade://aksel/*`, `arcade://project/*`, and capture-produced `arcade://preview/captures/*` resources. Read resources through MCP `resources/list`/`resources/read`; tool-only hosts can call `read_resource({ uri })` for the same content. Arcade source is virtual `arcade://...` content, not repository or filesystem files.
...
For the full smoke checklist, use an MCP client that exposes `resources/list` and `resources/read` (or an equivalent resource inspector). Tool-only MCP hosts should call `read_resource` for stable resources, diagnostics, source, Aksel snippets, and capture evidence.
