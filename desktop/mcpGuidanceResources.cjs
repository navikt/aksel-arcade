  ...
  switch (uri) {
    case 'arcade://desktop/start-here':
      return [
        ...
        '- **jsx vs hooks:** every Arcade page (and Global config) has two source tabs, `jsx` and `hooks`. The `jsx` source is inlined into `return ( … )`, so it must be a single JSX element/expression and must **never** be wrapped in `{ … }` (a leading `{` parses as an object literal and breaks the whole preview). In a page `hooks` tab, top-level hook calls (`useState`, etc.) are hoisted into that page component, so page state belongs there. Global config `hooks` is module scope: define shared custom hooks, helpers, constants, and components there, but never call hooks at its top level.',
