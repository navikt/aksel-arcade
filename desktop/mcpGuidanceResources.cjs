  ...
  switch (uri) {
    case 'arcade://desktop/start-here':
      return [
        ...
        '2. Read the source URIs listed in the manifest before editing existing work. Prefer `read_source` for editable source URIs so large JSX/Hooks source can be paged safely.',
        ...
        '- `arcade://desktop/apply-changes-operations` — the per-operation field matrix for `apply_changes`; `replace_source` still requires full replacement content.',
