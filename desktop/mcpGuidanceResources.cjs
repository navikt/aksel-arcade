  ...
  switch (uri) {
    case 'arcade://desktop/start-here':
      return [
        ...
        '3. For annotation review work, call `list_annotations` first or read `arcade://project/annotations` / `arcade://project/pages/{pageId}/annotations` for non-dead history and counts.',
        ...
        '- **Annotations are page-scoped review data.** `list_annotations` defaults to open work on the active page. The annotation resources keep non-dead history, while hidden-but-resolved targets still count as real work.',
