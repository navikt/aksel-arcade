...
Today an Arcade project is a single screen: one **JSX** body and one **Hooks** body that compile to one preview. Prototyping anything that spans more than one distinct screen — a flow with a landing page and a detail page, navigation between views, a broader journey with separate screens — is impossible without faking it inside a single component. Humans (and the agents that pair with Desktop Arcade) have nowhere to put a second screen, no way to move between screens in the preview, and no shared place for code that every screen needs (a top nav, shared constants, shared helpers).
...
When enabled, the **Arcade project source** becomes a permanent **Global config** plus an ordered set of **Arcade pages**. A new page panel (a left drawer that pushes the code editor) lets the user add pages, rename them, delete them, set a **Start page**, and switch the **Active page**. The JSX and Hooks tabs always edit whatever is selected in the panel — an Arcade page or the Global config. All pages plus the Global config compile into one running prototype with an injected `goToPage()` helper and an internal router, so a prototype can navigate between its own screens; Aksel anchors (`Link`, `LinkCard`) whose `href`/`to` is a page id navigate too. Navigating in the preview moves the panel selection and the editor with it, and vice versa. Stateful step flows that need shared in-memory data stay on one Arcade page with page-local React state; multi-page authoring is for genuinely distinct screens.
