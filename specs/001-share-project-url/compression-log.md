...
- **Measurement harness**: (Legacy) The retired `scripts/debug-share-length.mjs` runner used to spin up Vite + Playwright, load each template via the UI, and optionally pin codecs while logging payload stats. The new workflow relies on `npm run share-strategy-bench` plus manual registry tweaks when deterministic pinning is required.
...
- **Setup**: (Legacy) Used the old `scripts/verify-packed-share.mjs` harness (since removed) to load the "Oppsummeringsside for søknadsdialoger" template, pin `packed-deflate-b91`, open the Share popover, and copy the link via the real CopyButton before loading it inside a brand-new browser context.
- **Share URL**:
	```
...
- **Setup**: Dev server on `http://127.0.0.1:5173` with Summary template auto-loaded (Settings → “Oppsummeringsside for søknadsdialoger”). Strategy pinning relied on the now-removed `__akselShareDebug` shim before opening the Share popover.
- **Observed URL**:
	```
