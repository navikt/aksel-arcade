var inspectOverlayUtils = (() => {
  ...
  // public/inspect-overlays.js
  var inspect_overlays_exports = {};
  __export(inspect_overlays_exports, {
    clampRect: () => clampRect,
    computeGapRects: () => computeGapRects,
    computeSpacingOverlayRects: () => computeSpacingOverlayRects,
    createOverlayElements: () => createOverlayElements,
    hideOverlays: () => hideOverlays,
    overlayColors: () => overlayColors,
    parseSpacing: () => parseSpacing,
    renderGapLayers: () => renderGapLayers,
    renderSpacingLayers: () => renderSpacingLayers
  });
  var overlayColors = {
  ...
  var parseSpacing = (value) => {
    const parsed = parseFloat(value);
    return Number.isFinite(parsed) ? parsed : 0;
  };
  var clampRect = (rect, bounds) => {
  ...
  var computeSpacingOverlayRects = ({ boundingRect, margin, padding, viewport }) => {
  ...
  var addGapRect = (list, rect, bounds, dedupe) => {
  ...
  var computeGapRects = ({ layoutType, flexDirection, rowGap = 0, columnGap = 0, childrenRects = [], containerRect, viewport }) => {
    const vp = viewport ?? defaultViewport();
  ...
  var applyRect = (element, rect, color) => {
    if (!element) return;
    if (!rect) {
  ...
  var createOverlayElements = (doc = document) => {
  ...
  var renderSpacingLayers = (elements, spacingRects) => {
  ...
  var renderGapLayers = (elements, gapRects) => {
    if (!elements) return;
  ...
  return __toCommonJS(inspect_overlays_exports);
})();
